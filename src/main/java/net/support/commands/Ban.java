...
public class Ban extends Command {
    ...
    public Ban() {
        super("ban", "Herhangi bir kullanıcıyı uzaklaştırmanızı sağlar.", "b!ban (kullanıcı) (sebep)");
    }
    ...
    public void execute(Member user, Message message, String[] args) {
        if (user.hasPermission(Permission.BAN_MEMBERS)) {
            if (!(args.length < 2)) {
                ...
                user.getGuild().ban(member, 1)
                        .reason(sb.toString())
                        .queue();
                MessageEmbed embed = new EmbedBuilder()
                        .setAuthor(member.getUser().getName())
                        .setDescription("Oyuncu Banlandı.")
                        .setThumbnail(member.getUser().getAvatarUrl())
                        .addField("Banlayan kişi", user.getUser().getAsMention(), false)
            ...
            MessageEmbed embed = new EmbedBuilder()
                    .setAuthor("HATA")
                    .setDescription("Bu komutu kullanabilmek için ``BAN_MEMBERS`` yetkisine sahip olmalısın")
                    .setColor(Color.RED)
                    .build();
            message.getChannel().sendMessage(embed).queue();
        }
